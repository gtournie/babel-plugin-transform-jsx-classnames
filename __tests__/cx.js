...
describe('cx', function () {
  ...
  //   assert.equal(cx(new Class2()), 'classFromMethod')
  // })
  ...
  it('should make sure object with falsy value wipe out previous classes', function () {
    assert.equal(cx('foo', 0, null, undefined, true, 1, 'b', { foo: false }), '1 b')
    assert.equal(cx('foo', 'foobar', 'bar', { foo: false }), 'foobar bar')
    assert.equal(cx('foo foo', 'foobar', 'bar', { foo: false }), 'foobar bar')
    assert.equal(cx('foo', 'foo-bar', 'bar', { foo: false }), 'foo-bar bar')
    assert.equal(cx('foo', '-moz-foo-bar', 'bar', { foo: false }), '-moz-foo-bar bar')
    assert.equal(cx(['foo', ['b', ['c', { foo: false }]]]), 'b c')
  })
})
